    ...
    public function processar() {
        if ($this->request->getMethod() === 'post') {
            ...
            $validacao->setRules([
                ...
                'checkout.numero' => ['label' => 'Número', 'rules' => 'required|max_length[10]'],
                ...
                'checkout.bairro_slug' => ['label' => 'Endereço de entrega', 'rules' => 'required|string|max_length[20]'],
            ]);
            if (!$validacao->withRequest($this->request)->run()) {
            ...
            $pedido->produtos = serialize(session()->get('carrinho')); //serialize tem q ser em campo text
            ...
            if($forma->id == 1){
                if(isset($checkoutPost['sem_troco'])){
                    $pedido->observacoes = 'Ponto de referência: '.$checkoutPost['referencia']. ' - Número '.$checkoutPost['numero']. ' .Você informou que não precisará de troco ';
                }
                if(isset($checkoutPost['troco_para'])){
                    $trocoPara = str_replace(',', '', $checkoutPost['troco_para']);
                    
                    if($trocoPara < 1){
                        return redirect()->back()->with('atencao', 'Ao escolher dinheiro informe se precisa de troco!');
                    }
                    
                    $pedido->observacoes = 'Ponto de referência: '.$checkoutPost['referencia']. ' - Número '.$checkoutPost['numero']. ' .Você informou que precisará de troco para : R$ '.number_format($trocoPara,2);
                }
            }else{
                /* nao e forma de pagamento em dinheiro*/
                $pedido->observacoes = 'Ponto de referência: '.$checkoutPost['referencia']. ' - Número '.$checkoutPost['numero'];
            }
            
            $this->pedidoModel->save($pedido);
            
            $pedido->usuario = $this->usuario; //atribuir atibuto externo
            
            $this->enviaEmailPedidoRealizado($pedido);
            
            //limpar dados daqui pra baixo
...
// ----------------------------- funcao privada ----------------------------------
    ...
    private function enviaEmailPedidoRealizado(object $pedido) {
        ...
        $mensagem = view('Checkout/pedido_email', ['pedido' => $pedido]); // armazene na variavel mensagem a view reset email passando o valor de token
    ...
    private function buscaPedidoOu404(string $codigoPedido = null) {
        ...
        if (!$codigoPedido || !$pedido = $this->pedidoModel
                ...
                ->first()) {
            throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound("Nao encontramos o pedido $codigoPedido");
        }
        return $pedido;
    }
}

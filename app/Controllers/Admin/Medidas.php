...
class Medidas extends BaseController{
    ...
    public function atualizar($id = null) {
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            if ($medida->deletado_em != null) {
                return redirect()->back()->with('info', "A medida $medida->nome já encontra-se excluída. Portanto não é possive edita-lá.");
            }
    ...
    public function excluir($id = null) {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            return redirect()->to(site_url('admin/medidas'))->with('sucesso', "Categoria $medida->nome excluída com sucesso!");
        }
    ...
    public function desfazerExclusao($id = null) {
        ...
        if ($medida->deletado_em == null) {
            return redirect()->back()->with('info', 'Apenas medidas excluidas podem ser recuperadas');
        }
    ...
    private function buscaMedidaOu404(int $id = null) {
        ...
        if (!$id || !$medida = $this->medidaModel->withDeleted(true)->where('id',$id)->first()) {
            throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound("Nao encontramos a medida $id");
        }
        return $medida;
    }
